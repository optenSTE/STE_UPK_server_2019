...
output_measurements_order = ['T_degC', 'Fav_N', 'Fbend_N', 'Ice_mm']  # последовательность выдачи данных на ОСМ
DEFAULT_TIMEOUT = 10000
instrument_description_filename = 'instrument_description.json'

send_multi_packages = True  # отправка измерений пакетами (True) или по одному (False)
send_pause_sec = 0.2  # пауза между отправками пакетов
send_max_packages = 1000  # максимальное количество измерений в одной отправке
send_past_data = True  # отправка данных, полученных до соединения с ОСМ (если задание совпадает)
...
async def connection_handler(connection, path):
    ...
    if not master_connection:
        ...
        try:
            ...
            tmp_master_connection = connection
            logging.error('master connection did not response, new master connection')
        except:
    ...
    while True:
        ...
        try:
            ...
            # соединение закрыто
            logging.error(f'There is no connection while receiving data - websockets.exceptions.WebSocketException, {str(e.args)}')
        ...
        try:
            ...
            logging.error('wrong JSON message has been refused')
            json_msg.clear()
            return
        ...
        try:
            if len(averaged_measurements_buffer_for_OSM['data']) > 0 and json.dumps(instrument_description) != json.dumps(json_msg):
                ...
                # очистка буфера, если разрешено
                averaged_measurements_buffer_for_OSM['data'].clear()
            elif not send_past_data:
                averaged_measurements_buffer_for_OSM['data'].clear()
        finally:
            averaged_measurements_buffer_for_OSM['is_ready'] = True
...
async def instrument_init():
    ...
    logging.info(f'instrument_init(): ping IP {instrument_ip}')
    ...
    """
    logging.info(f'instrument_init(): class hyperion initialization')
    ...
    # остановка стриминга пиков
    if peak_stream is not None:
        logging.info(f'instrument_init(): peak_stream is not None - nothing to do, return')
        return
        logging.info(f'instrument_init(): stop_streaming()')
        peak_stream.stop_streaming()

    logging.info(f'instrument_init(): h1.set_active_full_spectrum_channel_numbers() for {active_channels} channels')
    try:
        await asyncio.wait_for(h1.set_active_full_spectrum_channel_numbers(active_channels), timeout=2)
    except asyncio.TimeoutError:
        logging.error('instrument_init(): h1.set_active_full_spectrum_channel_numbers() Timeout')

    # запуск стриминга пиков
    logging.info(f'instrument_init(): HCommTCPPeaksStreamer()')
    peak_stream = hyperion.HCommTCPPeaksStreamer(instrument_ip, loop, queue)
    logging.info(f'instrument_init(): peak_stream.stream_data()')
...
async def wls_to_measurements_coroutine():
    ...
    global wavelengths_buffer, measurements_buffer, last_temperature
                ...
                for (measurement_time, peaks_by_channel) in wavelengths_buffer['data'].items():
                    ...
                    for device_num, device in enumerate(devices):
                        ...
                        if wls:
                            ...
                            raw_measurements_buffer_for_disk['data'][measurement_time].append(device_output['T_degC'])
                            ...
                            if device_num == 1:
                                last_temperature = device_output['T_degC']

                        else:
                            for field_num, filed in enumerate(output_measurements_order):
                                devices_output3[1 + device_num * len(output_measurements_order) + field_num] = 0
            ...
            while send_msg != 'sent':
                ...
                try:
                    ...
                    if file_type == 'raw' and not Path(data_arch_file_name).is_file():
                        header = 'Timestamp, s\t'
                        for device in devices:
                            header += f'{device.name}_T, degC\t{device.name}_F1, N\t{device.name}_F2, N\t'
                        send_msg = header[:-1] + '\n' + send_msg
            ...
            try:
                ...
                if send_multi_packages:
                    for timestamp_msg in sorted(averaged_measurements_buffer_for_OSM['data'].keys(), reverse=True):
                        if timestamp_msg not in what_to_send and len(what_to_send) < send_max_packages:
                    ...
                    for timestamp_msg, measurements in what_to_send.items():
                        send_msg += '[' + ', '.join(
                            ['%.3f' % x for x in measurements]) + '], '
                    send_msg = send_msg[:-2]

                    if len(what_to_send) > 1:
                        send_msg = f'[{send_msg}]'
                else:
                    timestamp_msg = sorted(averaged_measurements_buffer_for_OSM['data'].keys(), reverse=True)[0]
                    send_msg = '[' + ', '.join(
                        ['%.3f' % x for x in averaged_measurements_buffer_for_OSM['data'][timestamp_msg]]) + ']'
...
async def clock_sync():
    clock_sync_interval_sec = 10

    while not h1:
        await asyncio.sleep(asyncio_pause_sec)
    while not h1.get_is_ready():
        await asyncio.sleep(asyncio_pause_sec)
    ...
    while True:
        await asyncio.sleep(asyncio_pause_sec)
        cur_time = datetime.datetime.now().timestamp()
        if h1.get_is_ready() and cur_time - last_sync_time > clock_sync_interval_sec:
            try:
                now = datetime.datetime.utcnow()
                await h1.set_instrument_utc_date_time(now)
                logging.info(f'Hyperion clock sync was made, new time {now}')
                last_sync_time = cur_time
            except Exception as e:
                logging.error(f'Some error during h1.set_instrument_utc_date_time - exception: {e.__doc__}')
'''

async def save_spectrum():
    spectrum_interval_sec = 300
    spectrum_devider = 4  # value of 5, will return every fifth value
            ...
            if await h1.get_is_ready() and cur_time - last_spectrum_time > spectrum_interval_sec:
                try:
                    ...
                    for channel in spectra_data.channel_map:
                        str_to_save = f'{str_to_save}{timestamp}\t{channel+1}\t' + '\t'.join(['%.2f' % pwr for pwr in spectra_data.data[channel]][::spectrum_devider]) + '\n'

                    data_arch_file_name = datetime.datetime.utcfromtimestamp(timestamp).strftime('%Y%m%d%H_spectra.txt')

                    # add single header for each output file
                    if not Path(data_arch_file_name).is_file():
                        header = 'Timestamp, s\tChannel\t'
                        for wl in spectra_data.wavelengths[::spectrum_devider]:
                            header += '%.3f\t' % wl
                        header = header[:-1]
                    ...
                    logging.info(f'Got spectrum')
                    last_spectrum_time = cur_time
                except Exception as e:
                    logging.error(f'Some error during h1.set_instrument_utc_date_time - exception: {e.__doc__}')
    except Exception as e:
        logging.error(f'Some error during save_spectrum() - exception: {e.__doc__}')
    finally:
...
async def heart_rate():
    ...
    buffers_names = ['wavelengths_buffer', 'measurements_buffer', 'averaged_measurements_buffer_for_OSM', 'averaged_measurements_buffer_for_disk', 'wls_buffer_for_saving']
...
if __name__ == "__main__":
    ...
    logging.basicConfig(format=u'%(filename)s[LINE:%(lineno)d]# %(levelname)-8s [%(asctime)s]  %(message)s',
                        level=logging.DEBUG,
                        handlers=[RotatingFileHandler(log_file_name, maxBytes=20*1024*1024, backupCount=10)])

    logging.info(u'Program starts v.20200131_Vladik')

    address, port = None, None
    try:
        address = sys.argv[1]
        port = int(sys.argv[2])
        # address, port = socket.gethostbyname_ex(socket.gethostname()), 7681  # адрес websocket-сервера
    except Exception as e:
        logging.critical(f'Restart programm with two arguments (address and port, space is delimiter) - exception: {e.__doc__}')
        print('Restart programm with two arguments (address and port, space is delimiter)')
        exit(0)
    ...
    if 1:
        ...
        message = 'Server {} has been started'.format((address, port))
    ...
    if 1 and Path(instrument_description_filename).is_file():
        # file exists
        ...
        try:
            ...
            logging.error(f'Some error during instrument decsription file reading; exception: {e.__doc__}')
        else:
