...
address, port = '10.117.93.27', 7681  # адрес websocket-сервера
...
async def instrument_init():
    ...
    await h1.set_active_full_spectrum_channel_numbers([1,2,3,4])
            ...
            if await h1.get_is_ready() and cur_time - last_spectrum_time > spectrum_interval_sec:
                ...
                try:
                    ...
                    for channel in spectra_data.channel_map:
                        str_to_save = f'{str_to_save}{timestamp}\t{channel}\t' + '\t'.join(['%.2f' % pwr for pwr in spectra_data.data[channel]][::4]) + '\n'
...
if __name__ == "__main__":
    ...
    logging.info(u'Program starts v.20200109_2')
